...
#load data
beam_3 = pd.read_csv('Beam3.dat', header=1) #pd.read_csv
#copy data into new names
mass = beam_3.iloc[:, 0].copy()
disp = beam_3.iloc[:, 1].copy()
# convert mass to force
force = mass * 9.81
# convert disp to meters
disp = (disp * 2.54) / 100
# generate predictions
p_3 = np.polyfit(force, disp, 1)
print(p_3)
#generate plots
...
disp_model = np.polyval(p_3, force_model)
...
ax.set_title('Displacement vs Force for Beam_3.dat (TK206)')
fig.tight_layout()
